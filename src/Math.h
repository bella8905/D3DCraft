...
struct SRay {
    ...
    glm::vec4 GetPointOnRay( const float& t_Scale ) const { return _Origin + t_Scale * _Dir; }
    SRay Transform( const glm::mat4& t_TransMat ) const { return SRay( t_TransMat * _Origin, t_TransMat * _Dir ); }
    SRay Translate( const glm::vec3& t_offset ) const { return SRay( glm::vec4( t_offset, 0.f ) + _Origin, _Dir ); }
